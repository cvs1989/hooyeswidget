...
function del_company($uid){
	...
	$rz=$db->get_one("select * from `{$_pre}company` where uid='$uid'");
	$rid = $rz['rid'];
	if(!$rz) return false;
	...
	//删除供求
	$query=$db->query("select * from `{$_pre}content_sell` where uid='$uid';");
		while($rs=$db->fetch_array($query)){
	...
	$query=$db->query("select * from `{$_pre}content_buy` where uid='$uid';");
		while($rs=$db->fetch_array($query)){
