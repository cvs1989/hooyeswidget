...
function make_ture_path($format,$rs){
	...
	elseif($format[SYS]=='normal'||$format[SYS]=='wn'){
		...
		}else if($format['SYS_type'] == 'business'){
			if($format['ctype'] == 1){
				$show_url = $webdb['www_url'].'/b/sell_bencandy.php?fid='. $rs['fid'] .'&id='. $rs['id'];
			}else if($format['ctype'] == 2){
				$show_url = $webdb['www_url'].'/b/buy_bencandy.php?fid='. $rs['fid'] .'&id='. $rs['id'];
			}else if($format['ctype'] == 3){
				$show_url = $webdb['www_url'].'/b/homepage.php?uid='. $rs['uid'];
			}
		}else{
