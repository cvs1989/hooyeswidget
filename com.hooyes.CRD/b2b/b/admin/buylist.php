...
if($job=="list")
{
	...
	$order=" posttime";
	$desc="DESC";
	$showpage=getpage("{$_pre}content_buy","WHERE $SQL","?job=list&fid=$fid&type=$type&ctype=$ctype&keyword=".urlencode($keyword),$rows,"");
	//$sort_fid=$Guidedb->Select("{$_pre}sort","fid",$fid,"?job=list");
	$query=$db->query("SELECT * FROM {$_pre}content_buy WHERE $SQL ORDER BY $order $desc LIMIT $min,$rows");
	...
	if(is_array($listdb) && count($listdb)>0){
		$ids=implode(",",$listdb);
		$db->query("update `{$_pre}content_buy` set yz=1 ,yztime='".time()."' where id in($ids);");		
	}
	if($id){
		$db->query("update `{$_pre}content_buy` set yz=1 ,yztime='".time()."' where id ='$id';");	
	}
	refreshto("$FROMURL","操作成功",1);
}elseif($jobs=="unyz"){
	if(is_array($listdb) && count($listdb)>0){
		$ids=implode(",",$listdb);
		$db->query("update `{$_pre}content_buy` set yz=0 ,yztime='' where id in($ids);");		
	}
	if($id){
		$db->query("update `{$_pre}content_buy` set yz=0 ,yztime='' where id ='$id';");	
	}
	refreshto("$FROMURL","操作成功",1);
	...
	if(is_array($listdb) && count($listdb)>0){
		$ids=implode(",",$listdb);
		$db->query("update `{$_pre}content_buy` set levels=1 ,levelstime='".time()."' where id in($ids);");		
	}
	if($id){
		$db->query("update `{$_pre}content_buy` set levels=1 ,levelstime='".time()."' where id ='$id';");	
	}
	refreshto("$FROMURL","操作成功",1);
	...
	if(is_array($listdb) && count($listdb)>0){
		$ids=implode(",",$listdb);
		$db->query("update `{$_pre}content_buy` set levels=0 ,levelstime='' where id in($ids);");		
	}
	if($id){
		$db->query("update `{$_pre}content_buy` set levels=0 ,levelstime='' where id ='$id';");	
	}
	refreshto("$FROMURL","操作成功",1);
	...
	if(is_array($listdb) && count($listdb)>0){
		$ids=implode(",",$listdb);
		$db->query("update `{$_pre}content_buy` set fid='$new_fid',`fname`='{$Fid_db[name][$new_fid]}' where id in($ids);");
	}
	refreshto("$FROMURL","操作成功",1);
