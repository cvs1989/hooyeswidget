...
if(!$job){
	...
	$query=$db->query("SELECT A.*,B.* FROM {$_pre}content_buy A INNER JOIN {$_pre}content_2 B ON B.id=A.id $where AND B.id!=''  ORDER BY A.posttime DESC LIMIT $min,$rows");
	
	while($rs=$db->fetch_array($query)){
		$rs[posttime]=date("Y-m-d",$rs[posttime]);
		$rs[yz]=!$rs[yz]?"<font color=red>审核中</font>":"已通过";
		$rs[mysort]=$rs[ms_id]?$mysort[$rs[ms_id]]:"未分类";
		$rs[picurl_min]=getimgdir($rs[picurl],2).".gif";
		...
		else $rs[tg_status]="<a href='../tg_buy.php?action=posttgnew&fid=$rs[fid]&id=$rs[id]' target='_blank'>推广</a>";
	...
	$showpage=getpage("{$_pre}content_buy A",$where,"?ms_id=$ms_id&keyword=$keyword2&tab=$tab",$rows);
	...
	if($id_db){
		$db->query("UPDATE `{$_pre}content_buy` SET  ms_id='$to_ms_id' WHERE id IN($id_db)");
	}
	refreshto("?","操作成功");
	...
	if($id_db){
		$db->query("UPDATE `{$_pre}content_buy` SET  posttime='$timestamp' WHERE id IN($id_db)");
	}
	refreshto("?","操作成功");
}
require(dirname(__FILE__)."/"."head.php");
require(dirname(__FILE__)."/"."template/buylist.htm");
