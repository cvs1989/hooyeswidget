...
$query=$db->query("select * from {$_pre}homepage_picsort where uid='$uid' order by orderlist desc limit 0,$webdb[company_picsort_Max];");
...
print <<<EOT
-->
	<a href='$Mdomain/member/?main=homepage_ctrl.php?atn=pic_upload'  target='_blank'>上传图片</a> | <a href='$Mdomain/member/?main=homepage_ctrl.php?atn=pic'  target='_blank'>管理</a> 
<!--
EOT;
}
print <<<EOT
...
print <<<EOT
-->
              <div style="float:left; width:130px; height:130px; text-align:center; padding:5px; margin:5px;"><a href="?uid=$uid&psid=$rs[psid]&m=pics&psid=$rs[psid]"><img src="$webdb[www_url]/$user_picdir/$rs[uid]/$rs[faceurl].gif"  style="border:2px #CCCCCC solid;" width="90" height="90" onerror="this.src='$Murl/images/default/userpicsortdefault.gif';"/><br />             <strong>$rs[name]</strong></a><strong></strong> 
              </div>
<!--
EOT;
}
...
EOT;
	...
	$min=($page-1)*$rows;
	$query=$db->query("select * from {$_pre}homepage_pic where uid='$uid' and psid='$psid' order by orderlist desc limit $min,$rows;");
	$i=0;
	while($rs=$db->fetch_array($query)){
	...
	$showpage=getpage("{$_pre}homepage_pic"," where uid='$uid' and psid='$psid'","?uid=$uid&m=pics&psid=$psid",$rows);
...
if($pid){
//getimagesize
	$thispicurl=str_replace($webdb[www_url],PHP168_PATH,$thispic[url]);
	if(file_exists($thispicurl)){
	$mysize=getimagesize($thispicurl);
	$w=$mysize[0];$w_2=intval($w/2);
	$h=$mysize[1];
...
print <<<EOT
-->
                  <span style="float:left; wdith:90px; height:90px; word-wrap: break-word; word-break: normal; text-align:center; margin:10px 10px 0px 10px; border:1px $color solid; padding:5px;"><a href="$Mdomain/homepage.php?uid=$rs[uid]&m=pics&psid=$rs[psid]&pid=$rs[pid]"><img src="$rs[url].gif"  width="90" height="90"  border=0 onerror="this.src='$Murl/images/default/userpicdefault.gif';" alt='$rs[title]'/></a>
