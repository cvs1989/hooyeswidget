...
elseif($job=='read')
{
	if( ereg("^pwbbs",$webdb[passport_type]) )
	{
		...
		if(!$rs){
			$db->query("UPDATE $TB[table] SET newpm=0 WHERE uid='$lfjuid'");
		}
	}
	elseif( ereg("^dzbbs",$webdb[passport_type]) )
	{
		$SQL="SELECT pmid AS mid,msgfrom AS username,msgfromid AS fromuid,msgtoid AS touid,new AS ifnew,subject AS title,dateline AS mdate,message AS content FROM {$TB_pre}pms WHERE `msgtoid`='$lfjuid' AND folder='inbox' AND pmid='$mid'";
		$db->query("UPDATE {$TB_pre}pms SET `new`=0 WHERE `msgtoid`='$lfjuid' AND folder='inbox' AND pmid='$mid'");
		$rs=$db->get_one("SELECT * FROM {$TB_pre}pms WHERE `msgtoid`='$lfjuid' AND folder='inbox' AND new=1");
		if(!$rs){
			$db->query("UPDATE $TB[table] SET newpm=0 WHERE uid='$lfjuid'");
		}
	}
...
elseif($job=='clean')
{
	...
	elseif( ereg("^dzbbs",$webdb[passport_type]) )
	{
		$SQL="DELETE FROM {$TB_pre}pms WHERE `msgtoid`='$lfjuid' AND folder='inbox' ";
		$db->query("UPDATE $TB[table] SET newpm=0 WHERE uid='$lfjuid'");
	}
	else
	{
	...
	if(!$step){
		require("head.php");
		echo "<CENTER><br><br><b style='font-size:16px;color:red'>不可恢复的操作，你确实要清空收件箱吗?<br><br><A HREF='?job=$job&step=2' >确定清空</A>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<A HREF='?job=list' >放弃</A></b></CENTER>";
		exit;
	}
	$db->query($SQL);
...
elseif($job=='send')
{
	if($step==2)
	{
		$rsdb=$db->get_one("SELECT $TB[uid] AS uid FROM $TB[table] WHERE $TB[username]='$touser' ");
		if(!$rsdb)
		{
			showerr("当前用户不存在");
		}
	...
	if($uid){
		@extract($db->get_one("SELECT $TB[username] AS username  FROM $TB[table] WHERE $TB[uid]='$uid'"));
	}
	require(dirname(__FILE__)."/"."head.php");
