...
foreach($parents as $v) $guidefid .= ' &gt; <a href="sell_list.php?fid='. $v['fid'] .'">'. $v['name'] .'</a>';
$guidefid .= ' &gt; <a href="sell_list.php?fid='. $fid .'">'. $bcategory->categories[$fid]['name'] .'</a>';
...
//列表----------------------------------------------------
	...
	if($fid_path){		//有选分类
		...
		$sf_list_fid_join = " INNER JOIN `{$_pre}sell_fid` sf $sf_list_force_index ON A.id = sf.id ";
		$sf_count_fid_join = " INNER JOIN `{$_pre}sell_fid` sf $sf_count_force_index ON A.id = sf.id ";
	...
	$showpage=getpage("{$_pre}content_sell A $A_count_force_index $sf_count_fid_join $com_join_","WHERE $SQL","sell_list.php?&fid=$fid&bid=$bid&listarea=$listarea&price1=$price1&price2=$price2&ordertype=$ordertype&mylistorder=$mylistorder&renzheng=$renzheng&agent=$agent&vip=$vip&keyword=".urlencode($keyword),$rows);
	
	$query=$db->query("SELECT A.*,B.*,C.title as owner_name,C.is_agent,C.is_vip,C.city_id,C.province_id,C.renzheng FROM {$_pre}content_sell A $A_list_force_index $sf_list_fid_join $com_join INNER JOIN {$_pre}content_1 B ON A.id=B.id WHERE $SQL $timelimit ORDER BY $order_by LIMIT $min,$rows ");
