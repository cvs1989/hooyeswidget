...
function changeClassName(slevel,my,fid,next,ctype,Murl)
{
	...
	for(var i=slevel+1;i<=4;i++){
		document.getElementById('cate_menu_'+String(i)).innerHTML="";	
	}

	document.getElementById("likeajaxform").src=Murl+'/post.php?action=getfidsonslist&fup='+fid+'&class='+String(slevel+1)+"&ctype="+ctype;
	...
	document.getElementById("parameters_postform_load").src=Murl+'/post.php?action=oladparametersform&fid='+fid+'&showid=parameters_postform&id='+document.getElementById('id').value;
	}
}
...
function setOldFid(me,Murl, ctype){
	...
	if(me.value!='0'){ 
		...
		me.selectedIndex=0;
		document.getElementById("parameters_postform_load").src=Murl+'/post.php?action=oladparametersform&fid='+option.value+'&showid=parameters_postform&id='+document.getElementById('id').value;
	}
}
