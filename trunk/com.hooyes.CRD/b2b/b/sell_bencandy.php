...
*获取信息正文的内容
**/
$base=$db->get_one("SELECT A.`htmlname`,B.* FROM `{$_pre}content_sell` A INNER JOIN `{$_pre}company` B ON A.uid=B.uid WHERE id='$id'");
...
$db->query("UPDATE {$_pre}content_sell SET hits=hits+1,lastview='$timestamp' WHERE id=$id");



$rsdb=$db->get_one("SELECT A.*,B.* FROM `{$_pre}content_sell` A INNER JOIN `{$_pre}content_1` B ON A.id=B.id WHERE A.id='$id'");
$rsdb[picurl]=$rsdb[picurl]?$webdb[www_url].'/'.$webdb[updir].'/'.$Imgdirname.'/'.$rsdb[picurl]:"";

$parents = $bcategory->get_parents($fid);
$guidefid = '';

foreach($parents as $v) $guidefid .= ' &gt; <a href="sell_list.php?ctype='. $ctype .'&fid='. $v['fid'] .'">'. $v['name'] .'</a>';
$guidefid .= ' &gt; <a href="sell_list.php?ctype='. $ctype .'&fid='. $bcategory->categories[$fid]['fid'] .'">'. $bcategory->categories[$fid]['name'] .'</a>';
...
//得到绑定的图片
$show_bd_pics=show_bd_pics("{$_pre}content_sell"," where id=$id",10);
...
require(getTpl("bencandy_1",$main_tpl));
