...
require(Mpath.'inc/categories.php');
	...
	$query=$db->query("SELECT * FROM {$_pre}tg_sell  $where ORDER BY tg_myid DESC LIMIT $min,$rows");
	while($rs=$db->fetch_array($query)){
		$overtime=intval( $rs[tg_posttime] + ($rs[tg_howlong]*60*60)    );
		if($overtime > $timestamp) $mytg_ids[]=$rs[tg_id]; 
		
		if($rs[tg_id] && $rs[tg_ctype]){
			$ctype_db=$db->get_one("SELECT A.picurl,A.my_price,B.quantity_type FROM {$_pre}content_sell A INNER JOIN {$_pre}content_1 B on B.id=A.id  WHERE A.id='$rs[tg_id]'; ");
	...
	$showpage=getpage("{$_pre}tg_sell",$where,"?action=tg",$rows);
	...
	if($action=='tg'){ //状态显示
		...
		$query=$db->query("SELECT A.*  FROM {$_pre}content_sell A  $where  LIMIT $min,$rows");
		...
		$showpage=getpage("{$_pre}content_sell A",$where,"?action=$action&keyword=".urlencode($keyword),$rows);

	}elseif($action=='posttgnew'){//推广新的。设置
		
		if(!$id || !$ctype)showerr("没有找到你要访问的页");		
		
		$rsdb=$db->get_one("SELECT * FROM {$_pre}content_sell WHERE id='$id' AND yz=1 LIMIT 1");
		...
		//看看是否已经在推广
		$tg_rsdb=$db->get_one("SELECT * FROM {$_pre}tg_sell WHERE tg_id='$id' AND tg_uid='$rsdb[uid]' AND (`tg_posttime`+(`tg_howlong`*60*60)) > $timestamp ");
		...
		//设置推广
		$db->query("INSERT INTO `{$_pre}tg_sell` ( `tg_myid` , `tg_id` , `tg_title` , `tg_price` , `tg_fid`, `tg_uid` , `tg_ctype` , `tg_posttime` , `tg_howmuch` , `tg_howlong`,`yz`,`posttime` ) VALUES ('', '$id', '$tg_title', '$rsdb[my_price]', '$to_fid', '$rsdb[uid]', '$rsdb[ctype]', '$tg_posttime', '$tg_howmuch', '$tg_howlong','$yz','$timestamp');");
		
		header("location:tg_sell.php?action=savetgnewok");
		echo "<META HTTP-EQUIV=REFRESH CONTENT='0;URL=tg_sell.php?action=savetgnewok'>";
		...
		$query=$db->query("SELECT * FROM {$_pre}tg_sell $where ORDER BY tg_howmuch DESC LIMIT 0,100");
		while($rs=$db->fetch_array($query)){
