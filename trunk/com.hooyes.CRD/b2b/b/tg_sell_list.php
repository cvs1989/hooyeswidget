...
if($from=='index'){
	...
	$query=$db->query("SELECT A.*,B.picurl,B.htmlname,B.fid,B.ctype,B.my_price,C.quantity_type FROM {$_pre}tg_sell A 
	INNER JOIN {$_pre}content_sell B ON B.id=A.tg_id
	INNER JOIN {$_pre}content_1 C ON C.id=B.id
	...
	while($rs=$db->fetch_array($query)){
		...
		if($webdb[bencandyIsHtml] && $rs[htmlname]){
			...
			$rs[url]=$Mdomain."/sell_bencandy.php?fid=$rs[fid]&id=$rs[tg_id]";
		}
		if($i<4){
			if($rs[picurl]) {
	...
	echo "document.getElementById('showtg_1').innerHTML=\"$show\";";
