...
/**
 *    购物车控制器，负责会员购物车的管理工作，她与下一步售货员的接口是：购物车告诉售货员，我要买的产品是我购物车内的产品
 *
 *    @author    Garbin
 */
...
class CartApp extends MallbaseApp
{
    ...
    function drop()
    {
        ...
        $this->json_result(array(
            ...
            'amount'=>  $cart_status['carts'][$store_id]['amount']   //返回指定企业的购物车状态
        ),'drop_item_successed');
    }

    /**
     *    更新购物车中产品的数量，以产品为单位，AJAX更新
     *
     *    @author    Garbin
     *    @param    none
     *    @return    void
    ...
    function update()
    {
        ...
        $this->json_result(array(
            ...
            'amount'    =>  $cart_status['carts'][$store_id]['amount']  //企业购物车总计
    ...
    function _get_carts($store_id = 0)
    {
        ...
        foreach ($cart_items as $item)
        {
            ...
            $carts[$item['store_id']]['amount']     += $item['subtotal'];   //各企业的总金额
            $carts[$item['store_id']]['quantity']   += $item['quantity'];   //各企业的总数量
            $carts[$item['store_id']]['goods'][]    = $item;
        }

        foreach ($carts as $_store_id => $cart)
        {
            $carts[$_store_id]['kinds'] =   count(array_keys($kinds[$_store_id]));  //各企业的产品种类数
        }
