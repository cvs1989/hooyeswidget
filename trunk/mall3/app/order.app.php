...
class OrderApp extends ShoppingbaseApp
{
    ...
    function index()
    {
        ...
        {
            /* 在此获取生成订单的两个基本要素：用户提交的数据（POST），产品信息（包含产品列表，产品总价，产品总数量，类型），所属企业 */
            ...
            $order_id = $order_type->submit_order(array(
                'goods_info'    =>  $goods_info,      //产品信息（包括列表，总价，总量，所属企业，类型）,可靠的!
            ...
            /* 下单完成后清理产品，如清空购物车，或将团购拍卖的状态转为已下单之类的 */
    ...
    function _get_goods_info()
    {
        ...
        switch ($_GET['goods'])
        {
            ...
            default:
                ...
                {
                    $return['quantity'] += $goods['quantity'];                      //产品总量
                    $return['amount']   += $goods['quantity'] * $goods['price'];    //产品总价
