...
define('FIELD_GOODS_NAME',      0); // 产品名称
define('FIELD_PRICE',              7); // 产品价格
define('FIELD_STOCK',              9); // 库存
define('FIELD_IF_SHOW',        20); // 是否上架
define('FIELD_RECOMMENDED', 21); // 推荐
define('FIELD_ADD_TIME',       22); // 发布时间
define('FIELD_DESCRIPTION', 24); // 产品描述
define('FIELD_LAST_UPDATE', 31); // 更新时间
define('FIELD_GOODS_IMAGE', 35); // 产品图片
define('FIELD_GOODS_ATTR',  26); // 产品属性
define('FIELD_SALE_ATTR',      36); // 销售属性（规格）
define('FIELD_CID',                   1); // 产品类目cid
/* 品牌申请状态 */
...
class My_goodsApp extends StoreadminbaseApp
{
    ...
    function batch_edit()
    {
        if (!IS_POST)
        {
             ...
             $this->assign('sgcategories', $this->_get_sgcategory_options());  // 企业分类
    ...
    function edit()
    {
        ...
        if (!IS_POST)
        {
            ...
            $default_goods_images = array(); // 默认产品图片
            $other_goods_images = array(); // 其他产品图片
   ...
   function spec_edit()
   {
        ...
        else
        {
            ...
            $default_spec = array(); // 更新产品中默认规格的信息
   ...
   function ajax_col()
   {
    ...
    function import_taobao()
    {
        ...
        else
        {
            ...
            foreach ($records as $record)
            {
                ...
                if ($remain !== false) // 如果企业等级有产品数量限制
                {
                    if ($remain <= 0)
                    {
                        if ($num_record == 0) // 还没有导入产品数就超过限制了
                        {
                ...
                {
                    if ($record[FIELD_GOODS_IMAGE]) // 企业等级无产品数量限制
                    {
                        $num_image++;
                    }
                }
    ...
    {
        $default_goods_image = Conf::get('default_goods_image'); // 商城默认产品图片
        if ($id > 0)
        {
