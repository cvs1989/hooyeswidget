...
function CjNewsIn($classid,$id,$checked,$uptime,$userid,$username){
	...
	while($r=$empire->fetch($sql))
	{
		...
		if($isqf==1)
		{
			$iqfsql=$empire->query("insert into {$dbtbpre}enewsqf(id,classid,checkuser,docheckuser,viewcheckuser,returncheck,notdocheckuser,checked) values('$id','$cr[newsclassid]','".addslashes($checkuser)."',',',',',0,',',0);");
		}
		//更新附件
