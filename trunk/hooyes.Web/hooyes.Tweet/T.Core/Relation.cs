...
namespace Tweet.Core
{
    public class Relation
    {
        public static int Save(RelationEntity dict)
        {
            ...
            string sql = @"INSERT INTO [dbo].[OAuth_Relation]([App], [UserID],[SubApp], [SubUserID],[UpdateTime]) 
                                  VALUES(@App, @UserID,@SubApp, @SubUserID,getdate())";
            ...
            if (count > 0)
            {
                sql = "update OAuth_Relation set SubUserID=@SubUserID  where App=@App and UserID=@UserID and SubApp=@SubApp ";
            }
        ...
        public static List<RelationEntity> Get()
        {
            ...
            string sqlSelect = "select * from OAuth_Relation order by id asc";
            SqlDataReader dr = SqlHelper.ExecuteReader(SqlHelper.Local, CommandType.Text, sqlSelect
                );
            while (dr.Read())
            {
