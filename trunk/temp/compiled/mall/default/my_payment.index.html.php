                ...
                <?php if (($this->_foreach['v']['iteration'] == $this->_foreach['v']['total'])): ?><tr class="line_bold"><?php else: ?><tr class="line"><?php endif; ?>
                    ...
                    <div class="padding1">
                     <?php if ($this->_var['payment']['installed']): ?>
                        <a href="#" ectype="dialog" uri="index.php?app=my_payment&amp;act=config&payment_id=<?php echo $this->_var['payment']['payment_id']; ?>&amp;code=<?php echo $this->_var['payment']['code']; ?>" dialog_id="my_payment_config" dialog_title="配置" dialog_width="600" class="add2_ico">配置</a> <a href="javascript:drop_confirm('卸载后所有使用该支付方式的订单将无法支付，若您只是不希望让用户可以选择该支付方式，可以使用“配置”将该支付方式禁用，您确定要卸载它吗？', 'index.php?app=my_payment&amp;act=uninstall&payment_id=<?php echo $this->_var['payment']['payment_id']; ?>');" class="delete">卸载</a>
                    <?php else: ?>
                        <a href="#" ectype="dialog" dialog_id="my_payment_install" dialog_title="安装" uri="index.php?app=my_payment&amp;act=install&code=<?php echo $this->_var['payment']['code']; ?>" dialog_width="600" class="add1_ico">安装</a>
                    <?php endif; ?>
                    </div>
                    </td>
                </tr>
