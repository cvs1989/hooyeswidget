...
namespace com.hooyes.crc.DAL
{
    ...
    public class RegisterAdmin
    {
        ...
        public bool SetPayStatus(string sn, bool payStatus)
        {
            bool rValue = true;
            try
            {
                string sql = @"update CRC_apply set Pay=@Pay
                             where sn=@sn";
                AccessHelper.ExecuteNonQuery(AccessHelper.conn, sql,
                    new OleDbParameter("@Pay", payStatus),
        ...
        public bool SetInvoicStatus(string sn, bool invoicStatus)
        {
            bool rValue = true;
            try
            {
                string sql = @"update CRC_apply set Invoice=@Invoice
                             where sn=@sn";
                AccessHelper.ExecuteNonQuery(AccessHelper.conn, sql,
                    new OleDbParameter("@Pay", invoicStatus),
                    new OleDbParameter("@sn", sn)
                    );
            }
            catch
