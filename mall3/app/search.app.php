...
define('ENABLE_SEARCH_CACHE', true); // 启用产品搜索缓存
define('SEARCH_CACHE_TTL', 3600);  // 产品搜索缓存时间
...
class SearchApp extends MallbaseApp
{
    ...
    {
        /* 组成查询条件 */
        $conditions = " g.if_show = 1 AND g.closed = 0 AND s.state = 1"; // 上架且没有被禁售，企业是开启状态,
        if (isset($param['keyword']))
        {
